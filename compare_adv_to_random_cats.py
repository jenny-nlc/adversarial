...
if __name__ == '__main__':
    ...
    parser.add_argument('--norm', default='inf', help="which norm to use: \
        currently <- {1,2,inf}")
    parser.add_argument('--N_mc', default=20, type=int, help="Number of MC forward \
        passes to use.")
    ...
    x_test = h5database['test']['X'][:args.N_data]
    y_test = h5database['test']['Y'][:args.N_data]
    h5database.close()
    model = load_model()
    input_tensor = model.input
    mc_model = U.MCModel(model, input_tensor, n_mc = args.N_mc )

    fg = FastGrad(CallableModelWrapper(model, 'probs'), sess=K.get_session()) #only generate adv examples for a single fwd pass
    print('Calculating Adversarial Direction')
    ...
    clip_min = x_test.min()
    clip_max = x_test.max()
    advs = (np.clip(x_test + ep * adv_etas, clip_min, clip_max) for ep in epsilons)
    ...
    perturbs =(np.clip(x_test + ep * eta, clip_min, clip_max) for ep in epsilons)
    ...
    save_path = U.create_unique_folder(os.path.join('save', 'cats_and_dogs_pertubations_{}_norm_run_'.format(ORD)))

    with open(os.path.join(save_path, 'cats_and_dogs_pertub_data.pickle'),'wb') as f:
        pickle.dump([epsilons,
                     ...
                     rnd_balds,
                     rnd_vars], f)
    with open(os.path.join(save_path, 'cats_and_dogs_labels.pickle'), 'wb') as f:
